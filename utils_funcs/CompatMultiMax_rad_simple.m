...
function [q]= ReCalc_q(Q, K_max, q_simple_max)
...
for i=1: numel(Qs)
    t= size(Qs{i},2);
    if t< t_max
        Qs{i} = interp1(t,Qs{i}', t_max)'; 
        Rads{i} = interp1(t, Rads{i}, t_max);
    end
end
