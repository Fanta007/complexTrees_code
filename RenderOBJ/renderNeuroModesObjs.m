...
%         color_neuroTree = [0.0, 0.0, 0.5];
    ...
    %     color_neuroTree = default_c(1, :);
        ...
        moveVec = [j*550, i* -1400, 0];  % 550, -5500
...
%                                     renderOptions{:});
        ...
        if i==1
            ...
            text(vertex(1, max_y_idx)+ moveVec(1), ...
                 vertex(2, max_y_idx)+ moveVec(2) + 30, ...
                 vertex(3, max_y_idx), ...
                 [num2str((j-1)*1-3), 'std'], 'FontSize', 15);
        end
...
% Mean info
S1 = sprintf('Modes, NeuronTree, %s, %d trees: %s, --- ', data_path, length(used_idxes), num2str(used_idxes) );
...
print(fig,'Modes_compNeuroTrees_chen','-dpng','-r800')
