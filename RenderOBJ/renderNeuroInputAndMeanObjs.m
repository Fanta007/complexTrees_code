...
%     color_neuroTree = default_c(1, :);
    ...
    if i == N
        moveVec = [mod(i-1, 17)*550, ceil(i/17)* -8000, 0];  % --- BotanTrees: [(i-1)*4, 0, 0], NeuroTrees; [(i-1)*4, 0, 0]
    else
        moveVec = [mod(i-1, 17)*550, ceil(i/17)* -8000, 0];
    end
...
print(fig,['InputAndMean_compNeuroTrees_', num2str(N-1), 'trees'],'-dpng','-r1000')
