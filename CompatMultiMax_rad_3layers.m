function [Qs] = CompatMultiMax_rad_3layers(Qs)
...
q_children_2Ls_all = cellfun(@(A)A.q_children, Qs, 'UniformOutput', false);
[K_max, max_id] = max(K_all);

% sk
sk_max = sk_all{max_id};
beta_rad_max = beta_rad_all{max_id};
q_children_2Ls_max = q_children_2Ls_all{max_id};

[sk_s, beta_rads] = cellfun(@(A) ReCalc_sk_beta_rad(A, K_max, sk_max, beta_rad_max), ...
                            Qs, 'UniformOutput', false);

% 2layers sub trees
q_children_2Ls = cellfun(@(A) ReCalc_2LsSubtrees(A, K_max, q_children_2Ls_max),...
                        Qs, 'UniformOutput', false);
...
% Qs = cell(1, length(Qs));
for i=1: length(Qs)
    Qs{i} = make_qCompTree_rad_3layers(q0s{i}, q_children_2Ls{i}, sk_s{i}, Qs{i}.b00_startP, ...
...
for i=1: length(Qs{1}.q_children)
    ...
    q_children_all = CompatMultiMax_rad_simple(q_children_all);
...
function [q_children_2Ls]= ReCalc_2LsSubtrees(Q, K_max, q_children_2Ls_max)

K = Q.K_sideNum;
q_children_2Ls = cell(1,K);
for i=1:K
    q_children_2Ls{i} = Q.q_children{i};
end

if K < K_max
    for i=K+1:K_max
        q_children_2Ls{i} = makeZeroST_rad(q_children_2Ls_max{i});
    end
end
