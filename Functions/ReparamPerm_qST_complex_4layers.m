...
function [G,qST1p, qST2p] = ReparamPerm_qST_complex_4layers(qST1,qST2, lam_m,lam_s,lam_p)
...
for i=1:K1
    ...
    for j=1:K2
        ...
        [G, ~] = ReparamPerm_qST_complex_3layers(qST1.q_children{i}, qST2.q_children{j}, lam_m,lam_s,lam_p);
        Eside = sqrt(G.E);
...
for i = 1: numel(qST1p.q_children)
    ...
    % numel(qST2p.q_children{i}.q) .
    [G1, qST1p.q_children_p{i}, qST2p.q_children_p{i}] = ReparamPerm_qST_complex_3layers(qST1p.q_children{i}, qST2p.q_children{i},lam_m,lam_s,lam_p);
%     [G, qST1p.q_children_p{i}] = ReparamPerm_qST(qST1p.q_children{i}, qST2p.q_children{j}, lam_m,lam_s,lam_p);
        
%     if numel(qST1p.q_children_p{i}.q) > numel(qST2p.q_children{i}.q)
%         qST2p.q_children_p{i} = AddVitualTrees_3layers(qST1p.q_children_p{i}, qST2p.q_children{i});
%     else
