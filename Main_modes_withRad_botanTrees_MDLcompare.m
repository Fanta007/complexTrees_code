...
% Top-1:
ratio_top1 = sum(E_vec(1:1))/sum(E_vec);

% Top-2:
ratio_top2 = sum(E_vec(1:2))/sum(E_vec);

% Top-3:
ratio_top3 = sum(E_vec(1:3))/sum(E_vec);

% Top-4:
ratio_top4 = sum(E_vec(1:4))/sum(E_vec);

% Top-5:
ratio_top5 = sum(E_vec(1:5))/sum(E_vec);

% Top-6:
ratio_top6 = sum(E_vec(1:6))/sum(E_vec);

fprintf('ratio top1 to top6 is: %.2f,%.2f,%.2f,%.2f,%.2f,%.2f\n',...
                                ratio_top1, ratio_top2, ratio_top3, ratio_top4, ratio_top5, ratio_top6)

%### Go to "forMDLcomputation/Result2-treePCA_linearMethod/Main_v2.m" to make comparisons
