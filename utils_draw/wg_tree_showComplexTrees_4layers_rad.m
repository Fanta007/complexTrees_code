...
for i=1: numel(CT)
    ...
    y_added = ceil(i/19) * -0;
    x_added = (mod(i, 19)+1) * 3;
% %     x_added = (mod(i, 19)+1) * 4.5;
%     
    ...
    plot3(X0 , Y0 , Z0 , 'Color',c(1,:), 'LineWidth', 2); hold on;
%     text(X0(1), Y0(1), Z0(1), num2str(i)); hold on;
    count = count+1;

    for j=1: numel(CT{i}.beta)
        clear X Y Z
        
        X0 = CT{i}.beta_children{j}.beta0(1, :)- first_pt_x + x_added;
        Y0 = CT{i}.beta_children{j}.beta0(2, :)- first_pt_y + y_added;
        Z0 = CT{i}.beta_children{j}.beta0(3, :)- first_pt_z;
        R0 = CT{i}.beta_children{j}.beta0(4, :);
        
%         drawTubulars([X0', Y0', Z0',R0'], 3);
        hold on;
%         text(X0(end), Y0(end), Z0(end), [num2str(i),'-',num2str(j)]); hold on;
        
        plot3(X0, Y0, Z0, 'Color',c(1,:), 'LineWidth', 2); hold on;
        
%         text(X0(1), Y0(1) , Z0(1), [num2str(i),'-', num2str(j)]); hold on;
        
        count = count+1;
        
        for k = 1: numel(CT{i}.beta_children{j}.beta)
            clear X Y Z
%             X0 = CT{i}.beta_children{j}.beta{k}(1, :)- first_pt_x + x_added;
%             Y0 = CT{i}.beta_children{j}.beta{k}(2, :)- first_pt_y + y_added;
%             Z0 = CT{i}.beta_children{j}.beta{k}(3, :)- first_pt_z;
%             R0 = CT{i}.beta_children{j}.beta{k}(4, :);
%             
            X0 = CT{i}.beta_children{j}.beta_children{k}.beta0(1, :)- first_pt_x + x_added;
            Y0 = CT{i}.beta_children{j}.beta_children{k}.beta0(2, :)- first_pt_y + y_added;
            Z0 = CT{i}.beta_children{j}.beta_children{k}.beta0(3, :)- first_pt_z;
            R0 = CT{i}.beta_children{j}.beta_children{k}.beta0(4, :);
        
%             drawTubulars([X0', Y0', Z0',R0'], 3);
            hold on;
            plot3(X0, Y0, Z0, 'Color',c(1,:), 'LineWidth', 2); hold on;

%             text(X0(1), Y0(1), Z0(1), [num2str(i),'-', num2str(j), '-',num2str(k)]); hold on;
...
%             
            for t = 1: numel(CT{i}.beta_children{j}.beta_children{k}.beta)
                ...
                clear X Y Z
                X0 = CT{i}.beta_children{j}.beta_children{k}.beta{t}(1, :)- first_pt_x + x_added;
                Y0 = CT{i}.beta_children{j}.beta_children{k}.beta{t}(2, :)- first_pt_y + y_added;
                Z0 = CT{i}.beta_children{j}.beta_children{k}.beta{t}(3, :)- first_pt_z;
                R0 = 0.01* ones(1, numel(X0));
%                 if j == -1
%                     plot3(X- first_pt_x + x_added, Y- first_pt_y+ y_added, Z- first_pt_z, 'Color',c(count,:), 'LineWidth', 2); hold on;
%                 else
%                     plot3(X- first_pt_x + x_added, Y- first_pt_y+ y_added, Z- first_pt_z, 'Color',c(count,:), 'LineWidth', 2); hold on;
...
%                 drawTubulars([X0', Y0', Z0',R0'], 3);
                ...
                plot3(X0, Y0, Z0, 'Color',c(1,:), 'LineWidth', 1); hold on;
    ...
    % --- write the geodesic sample into a obj file ---
%     complexTree2Obj(CT{i}, num2str(i), 0, 0, 0);
% %     
 
end

% savefig('Geodesic_complexTrees.fig');
...
avi_obj = VideoWriter(video_name);%新建叫example.avi的文件
open(avi_obj); %打开
