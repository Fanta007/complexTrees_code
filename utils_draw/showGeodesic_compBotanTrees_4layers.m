...
for i=1: numel(CT)
    ...
    y_added = ceil(i/19) * -0;
    x_added = (mod(i, 19)+1) * 3;      % (mod(i, 19)+1) * 3
    ...
    plot3(X0 -first_pt_x + x_added, Y0 - first_pt_y + y_added, Z0 - first_pt_z, 'Color',default_c(1,:), 'LineWidth', 1); hold on;
%     text(X0(1) -first_pt_x + x_added, Y0(1) - first_pt_y + y_added, Z0(1) - first_pt_z, num2str(i)); hold on;
    ...
    for j=1: numel(CT{i}.beta)
        ...
        Y = CT{i}.beta{j}(2, :);
        Z = CT{i}.beta{j}(3, :);
        plot3(X- first_pt_x + x_added, Y- first_pt_y+ y_added, Z- first_pt_z, 'Color',default_c(1,:), 'LineWidth', 1); hold on;
...
saveas(gcf,[num2str(idx1),'with', num2str(idx2), '-Geod_CompBotanTrees_4layers.png'])
