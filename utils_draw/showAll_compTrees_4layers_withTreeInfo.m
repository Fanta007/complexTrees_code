...
for i=1: numel(CT)
    ...
    y_added = ceil(i/30) * (-1800);
    x_added = (mod(i-1, 30)) * 400;
...
%     plot3(X0 -first_pt_x + x_added, Y0 - first_pt_y + y_added, Z0 - first_pt_z, 'Color',c(1,:), 'LineWidth', 2); hold on;
    ...
    for j=1: numel(CT{i}.beta)
        ...
        R= CT{i}.beta_rad{j};
         ...
         plot3(X, Y, Z, 'Color',c(1,:), 'LineWidth', 1); hold on;
%         plot3(X- first_pt_x + x_added, Y- first_pt_y+ y_added, Z- first_pt_z, 'Color',c(count,:), 'LineWidth', 2); hold on;
    ...
    % Save
    if i==30 || i==70
        saveas(gcf,'All_compTrees_4layers.png');
    end
    
    % Write idx
    text(X0(1), Y0(1)-200, Z0(1), num2str(i), 'FontSize', 10, 'Color', 'r'); hold on;
    text(X0(1), Y0(1)-300, Z0(1), ['L:', num2str(amount_layer(i))], 'FontSize', 10);
    text(X0(1), Y0(1)-500, Z0(1), ['B:', num2str(amount_branch(i))], 'FontSize', 10);
        
end

saveas(gcf,'All_compTrees_4layers_withTreeInfo.png')
